...
class YoloToYoloFormatter:
	...
	@staticmethod
	def mod_images(yaml_dir: str, splits: list, mod_func):
		'''
	...
	@staticmethod
	def get_img_dir(yaml_dir: str, split: str):
		'''
	...
	@staticmethod
	def get_label_dir(yaml_dir: str, split: str):
		'''
	...
	def remove_non_person_classes(yaml_dir: str):
		'''
		returns the directory of where the validation labels are stored
		...
		for split in ["train", "val", "test"]:
			img_dir = YoloToYoloFormatter.get_img_dir(yaml_dir, split)
			lbl_dir = YoloToYoloFormatter.get_label_dir(yaml_dir, split)

			# Go through all files in chosen the directory.
			for file_name in os.listdir(lbl_dir):
				...
				if file_name.endswith('.txt'):
					...
					# Open the label file
					file_path = os.path.join(lbl_dir, file_name)
					with open(file_path , 'r') as file:
						...
						for line in file:
							...
							# Record the class of every object in the file.
							Label = line[0]
							Labels.append(Label)

						file.close()
						
						# Identify the class (Replace the number with the number of the class you want to retain).
						if '0' not in Labels:
							print('no people')
						
							# Remove the files that don't contain the choses class(es).
							os.remove(file_path)
							os.remove(img_path)